...
async function checkIfBranchExistsInRemote(
	...
	try {
		...
		const branchSummary = await git.branch(["-r", "--no-abbrev"]);
...
async function deleteRemoteBranch(branchName: string): Promise<void> {
	...
	try {
		const remoteBranchExists = await checkIfBranchExistsInRemote(branchName);
...
async function renameGitBranch(
	...
	try {
		...
		await git.checkout(currentBranchName); // Switch to the current branch
		await git.branch(["-m", newBranchName]); // Rename the branch locally
		...
		if (isRemoteBranch) {
			...
			await git.push("origin", newBranchName); // Push the renamed branch to the remote repository
